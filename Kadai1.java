	...
	public static void main(String[] arg){
		...
		for(int i = 0; i < testNumber; i++){
			...
			for(int j = 0; j < Students.length; j++){
				...
				if(result < total_Score[j]){
					result = total_Score[j];
					total_Name = name[j];
			...
			// total_Average = total_Average / Students.length;
			}
		}
		
		for(int k = 0; k <testNumber	;k++){  //各教科の（最高得点、最高得点者、平均点）を出力
			System.out.println("------------------------------------------------");
		...
		System.out.println("------------------------------------------------"); //クラスの合計点の（最高得点、最高得点者、全体の平均点）を出力
		System.out.println("合計点での最高得点は:" + result + "点です。");
		System.out.println("最高得点者は"+ total_Name + "さんです。");
